...
const unZipMVR = async zip => {
  ...
  zipContent = entries
  Object.entries(zipContent).forEach(async ([name, entry]) => {
...
const readFile = async fileToLoad => {
  return await zipContent[fileToLoad].blob()
}
...
const extractRotationAndPosition = matrix => {
...
const read3DMVR = async () => {
  ...
  // Get all SymDefs first
  const symdefs = MVRDocument.querySelectorAll('Symdef')

  let Symdef = []

  symdefs.forEach(def => {
    const uuid = def.getAttribute('uuid')
    const geometries3D = def.querySelectorAll('Geometry3D')
    Symdef[uuid] = []
    geometries3D.forEach(geometry3D => {
      Symdef[uuid].push(geometry3D.getAttribute('fileName'))
    })
  })
  ...
  await layers.forEach(layer => {
    ...
    const sceneObjects = layer.querySelectorAll('SceneObject')

    sceneObjects.forEach(sceneObject => {
      const matrix = sceneObject.querySelector('Matrix')
      const transformation = matrix ? extractRotationAndPosition(matrix.innerHTML) : { rotation: { x: 0, y: 0, z: 0 }, position: { x: 0, y: 0, z: 0 } }

      const objectColor = sceneObject.querySelector('Color') ? xyY2RGB(sceneObject.querySelector('Color').innerHTML) : '#00FF00'

      const pos = transformation.position
      const rot = transformation.rotation

      const loader = new GLTFLoader()

      const Geometry3D = sceneObject.querySelector('Geometry3D')
      const Symbol = sceneObject.querySelector('Symbol')

      let modelToLoad = []

      if (Geometry3D) {
        modelToLoad.push(readFile(Geometry3D.getAttribute('fileName')))
      } else if (Symbol) {
        const uuid = Symbol.getAttribute('symdef')
        if (Symdef[uuid] && Symdef[uuid].length > 0) {
          Object.entries(Symdef[uuid]).forEach(model => {
            modelToLoad.push(readFile(model[1]))
        })
        }
      }

      if (Geometry3D || Symbol) {
        Object.entries(modelToLoad).forEach(modelFile => {
          loader.load(modelFile[1], gltf => {
            const model = gltf.scene
            model.position.set(pos.x / 100, pos.y / 100, pos.z / 100)
            model.rotation.set(THREE.MathUtils.degToRad(rot.x), THREE.MathUtils.degToRad(rot.y), THREE.MathUtils.degToRad(rot.z))
            //model.MeshStandardMaterial({ color: parseInt(objectColor.replace('#', '0x'), 16) })
            groupObject.add(model)
          })
        })
      }
    })
    ObjectGroup.add(groupObject)

    scene.add(ObjectGroup)
    const group = new THREE.Group()

    const fixtures = layer.querySelectorAll('Fixture')

    fixtures.forEach(fixture => {
      const transformation = extractRotationAndPosition(fixture.querySelector('Matrix').innerHTML)
      ...
      cube.position.set(pos.x / 100, pos.y / 100, pos.z / 100)
      cube.rotation.set(THREE.MathUtils.degToRad(rot.x), THREE.MathUtils.degToRad(rot.y), THREE.MathUtils.degToRad(rot.z))
...
const readMVR = () => {
  ...
  button3D.addEventListener('click', async () => {
